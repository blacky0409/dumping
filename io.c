...
static unsigned int __do_perform_io(int sqid, int sq_entry, struct nvmev_dev *nvmev_vdev)
{
	...
	while (remaining) {
		...
		if(!first_io && nvmev_vdev->hugemap_checker == 1){
			if(old_paddr + PAGE_SIZE != paddr){
				nvmev_vdev->conti_counter++;
			}
			nvmev_vdev->count_page++;
		}
...
static unsigned int __do_perform_io_using_dma(int sqid, int sq_entry, struct nvmev_dev *nvmev_vdev)
{
	struct nvmev_submission_queue *sq = nvmev_vdev->sqes[sqid];
	struct nvme_rw_command *cmd = &sq_entry(sq_entry).rw;
	size_t offset;
...
void NVMEV_IO_WORKER_INIT(struct nvmev_dev *nvmev_vdev)
{
	...
	for (worker_id = 0; worker_id < nvmev_vdev->config.nr_io_workers; worker_id++) {
		...
		snprintf(worker->thread_name, sizeof(worker->thread_name), "nvmev_io/%d:%d",
			 nvmev_vdev->dev_id, worker_id);
