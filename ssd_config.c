...
void load_conv_configs(struct ftl_configs *ftl_cfgs)
{
	...
	ftl_cfgs->SSD_PARTITIONS = 4; //original =4
	ftl_cfgs->NAND_CHANNELS = 8; //original =8
...
void load_zns_configs(struct ftl_configs *ftl_cfgs)
{
	...
	ftl_cfgs->FLASH_PAGE_SIZE = KB(16);
	...
	//ftl_cfgs->NAND_ERASE_LATENCY = 0;
	ftl_cfgs->NAND_ERASE_LATENCY = 3500000;
...
/*static int SUPPORTED_SSD_TYPE(uint32_t type, struct ftl_config *ftl_cfgs){
	if(ftl_cfgs->NS_SSD_TYPE==type)
		return true;
	return false;
}*/
