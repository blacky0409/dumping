...
//	printk("request_time offs %d  request time %lld cur_time %lld\n", request_time_offs, request_time, cur_time);
...
uint64_t chmodel_request_copy(struct channel_model *ch, uint64_t request_time, uint64_t length,
			 struct nvmev_dev *nvmev_vdev)
{
	uint64_t cur_time = __get_wallclock(nvmev_vdev);
	uint32_t pos, next_pos;
	...
	while (1) {
		consumed_credits = (remaining_credits <= ch->max_credits) ?
						 remaining_credits :
						 ch->max_credits;
		remaining_credits -= consumed_credits;
