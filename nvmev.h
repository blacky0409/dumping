...
struct nvmev_config {
	...
	uint64_t count;
	uint64_t ch_die_counter;
	uint64_t ch_die_latency;
	bool checkpoint;
	bool * first_io;
...
struct nvmev_dev {
	...
	struct kobj_attribute *sysfs_phfrag;
	struct kobj_attribute *sysfs_chdie;
	struct kobj_attribute *sysfs_hugemap;
	struct kobj_attribute *sysfs_qing;
	...
	long degree_frag; /*degree of physical fragmentation*/
	int max_con;
	long num_list;
...
struct write_pattern_stats {
    uint64_t prev_lpn;
    uint64_t total_writes;
    uint64_t sequential_writes;
    uint64_t large_jump_writes;
    uint64_t lpn_delta_sum;
	int start;
};
...
struct nvmev_ns {
	...
	struct write_pattern_stats * stats;
