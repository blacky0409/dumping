...
uint64_t ssd_advance_pcie_copy(struct ssd *ssd, uint64_t request_time, uint64_t length)
{
	struct channel_model *perf_model = ssd->pcie->perf_model;
	return chmodel_request_copy(perf_model, request_time, length, ssd->p_ns->p_dev);
}
				  ...
				  int transfer_size_KB)
{
	...
	nsecs_latest += spp->fw_wbuf_lat1 * DIV_ROUND_UP(length, KB(transfer_size_KB));
...
uint64_t ssd_advance_nand(struct ssd *ssd, struct nand_cmd *ncmd)
{
	...
	case NAND_READ:
		...
		//printk(KERN_INFO "(%d,%d) cur : %lld io start : %lld lun_avail_time : %lld\n",ppa->g.ch,ppa->g.lun,nand_stime,cmd_stime,lun->next_lun_avail_time);
		if(ssd->checking && !lun->first_io){
			uint64_t delay = nand_stime - cmd_stime;
			if(delay > 0){
//				printk(KERN_INFO "(%d,%d) delay : %lld\n",ppa->g.ch, ppa->g.lun,delay);
				ssd->ch_die_counter++;
				ssd->ch_die_latency+= delay;
			}	
//			else{
//				printk(KERN_INFO "(%d,%d) no delay\n",ppa->g.ch, ppa->g.lun);
//			}

			if(delay < 0) delay = 0;

			if(ncmd->arrive_time < 0 || ncmd->arrive_time > delay){
				ncmd->arrive_time = delay;
			}

			lun->first_io=true;	
		}

//		printk("%lld, %lld",lun->next_lun_avail_time,cmd_stime);
...
uint64_t max_cstime_vs_ioclock(uint64_t cmd_stime, struct ssd *ssd)
{
	return (cmd_stime == 0) ? __get_ioclock(ssd) : cmd_stime;
}
