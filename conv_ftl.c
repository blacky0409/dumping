...
{
	struct line *node, *tmp;
	struct line *node_2, *tmp_2;
	pqueue_free(conv_ftl->lm.victim_line_pq);
    list_for_each_entry_safe(node, tmp, &conv_ftl->lm.free_line_list, entry) {
        list_del_init(&node->entry);
    }
    list_for_each_entry_safe(node_2, tmp_2, &conv_ftl->lm.full_line_list, entry) {
        list_del_init(&node_2->entry);
    }
...
static void advance_write_pointer(struct conv_ftl *conv_ftl, uint32_t io_type)
{
	...
	if(io_type==USER_IO){
		uint64_t idx = wpp->ch * spp->luns_per_ch + wpp->lun;

	// should convert gc version
		check_addr(conv_ftl->page_counter[idx], spp->pgs_per_blk);
		conv_ftl->page_counter[idx]++;
		if ((conv_ftl->page_counter[idx] % spp->pgs_per_oneshotpg) != 0)
			goto out;

		int start_ch = wpp->ch;
	    int start_lun = wpp->lun;
		...
		do{
			...
			idx = wpp->ch * spp->luns_per_ch + wpp->lun;
	        if (conv_ftl->page_counter[idx] < spp->pgs_per_blk) {
	            goto out;
	        }

		} while(wpp->ch != start_ch || wpp->lun != start_lun);
		uint64_t i,j;

    	 for (i = 0; i < spp->nchs; i++) {
    	     for (j = 0; j < spp->luns_per_ch; j++) {
    	         uint64_t reset_idx = i * spp->luns_per_ch + j;
    	         NVMEV_ASSERT(conv_ftl->page_counter[reset_idx] == spp->pgs_per_blk);
    	         conv_ftl->page_counter[reset_idx] = 0;
    	     }
    	 }
		...
		NVMEV_ASSERT(conv_ftl->page_counter[0] == 0);
		NVMEV_ASSERT(wpp->lun == 0);
		NVMEV_ASSERT(wpp->ch == 0);
		NVMEV_ASSERT(wpp->pl == 0);
	out:
		idx = wpp->ch * spp->luns_per_ch + wpp->lun;
		wpp->pg = conv_ftl->page_counter[idx];
...
static struct ppa get_new_page(struct conv_ftl *conv_ftl, uint32_t io_type)
{
	...
	//printk(KERN_INFO "blk: %d ch: %d lun: %d pg: %d\n",wp->blk,wp->ch,wp->lun,wp->pg);
...
void conv_init_ftl(struct conv_ftl *conv_ftl, struct convparams *cpp, struct ssd *ssd)
{
	/*copy convparams*/
	conv_ftl->cp = *cpp;
...
void conv_remove_ftl(struct conv_ftl *conv_ftl)
{
			 ...
			 uint32_t cpu_nr_dispatcher, struct ftl_configs *ftl_cfgs)
{
	...
	for (i = 0; i < ns->nr_parts; i++) {
		...
		conv_ftls[i].page_counter= vzalloc(sizeof(int64_t) * spp.nchs * spp.luns_per_ch);
		ssd->p_ns = ns;
		ssd->checking=false;
	}
...
void conv_remove_namespace(struct nvmev_ns *ns)
{
	...
	for (i = 1; i < nr_parts; i++) {
		...
		vfree(conv_ftls[i].page_counter);
	}
...
static bool conv_read(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	uint32_t xfer_size, i,q,w;
	uint32_t nr_parts = ns->nr_parts;
	struct ssd *ssd;

	for(q=0;q < ns->nr_parts; q++){
		ssd = conv_ftls[q].ssd;
		for(w=0; w < spp->nchs; w++){
			for(i=0; i < ssd->ch[w].nluns; i++){
				ssd->ch[w].lun[i].first_io = false;
			}
		}
	}
	//printk(KERN_INFO "size : %lld, real size : %lld\n",nr_lba,nr_lba/spp->secs_per_pg);
	struct ppa prev_ppa;
	struct nand_cmd srd = {
		.type = USER_IO,
		.cmd = NAND_READ,
...
//		printk(KERN_INFO "hi\n");
	for (i = 0; (i < nr_parts) && (start_lpn <= end_lpn); i++, start_lpn++) {
		...
		for (lpn = start_lpn; lpn <= end_lpn; lpn += nr_parts) {
			...
			if (xfer_size > 0) {
				...
				srd.ppa = &prev_ppa;
		//		printk(KERN_INFO "nr : %d ",lpn % nr_parts);
	...
	ns->p_dev->queueing_wait += srd.arrive_time;
	ret->nsecs_target = nsecs_latest;
	ret->status = NVME_SC_SUCCESS;
	return true;
}

void track_write(struct write_pattern_stats *stats, uint64_t start_lpn) {
    if (stats->total_writes > 0) {
        int64_t delta = (int64_t)start_lpn - (int64_t)stats->prev_lpn;
        uint64_t abs_delta = delta < 0 ? -delta : delta;

        if (abs_delta <= 8)
            stats->sequential_writes++;
        if (abs_delta > 1024)
            stats->large_jump_writes++;

        stats->lpn_delta_sum += abs_delta;
    }
    stats->prev_lpn = start_lpn;
    stats->total_writes++;
}
...
static bool conv_write(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	if(ns->stats->start==1)
		track_write(ns->stats,start_lpn);
	...
	nsecs_latest = ssd_advance_write_buffer(conv_ftl->ssd, req->nsecs_start,
						LBA_TO_BYTE(nr_lba), transfer_4);
...
static struct rewrite_pointer * advance_write_pointer_cleanver(struct conv_ftl *conv_ftl, uint64_t degree,bool no_return, int * counter, int *conti,bool final)
{
	uint32_t io_type=USER_IO;
	struct ssdparams *spp = &conv_ftl->ssd->sp;
	struct line_mgmt *lm = &conv_ftl->lm;
	struct write_pointer *wpp = __get_wp(conv_ftl, io_type);
	uint32_t random_value;
	uint64_t idx = wpp->ch * spp->luns_per_ch + wpp->lun;
	struct rewrite_pointer * r = NULL; 

	NVMEV_DEBUG_VERBOSE("current wpp: ch:%d, lun:%d, pl:%d, blk:%d, pg:%d\n", wpp->ch, wpp->lun,
			    wpp->pl, wpp->blk, wpp->pg);
	
	//printk(KERN_INFO "ch: %d lun: %d pg : %d counter: %d\n",wpp->ch, wpp->lun, wpp->pg, page_counter[idx]);
	check_addr(conv_ftl->page_counter[idx], spp->pgs_per_blk);
	(*counter)++;
	conv_ftl->page_counter[idx]++;
	if ((conv_ftl->page_counter[idx] % spp->pgs_per_oneshotpg) != 0)	
	...
	if(!final){
		if((*conti) >= 0){
			(*conti)++;
			if((*conti) >= spp->max_con)
				(*conti) = -1;
			if((*counter)>= 16 * spp->pgs_per_oneshotpg){
				r = vmalloc(sizeof(struct rewrite_pointer)); 
				r->ch = wpp->ch;
				r->lun = wpp->lun;
			}
			else if(conv_ftl->page_counter[idx] < spp->pgs_per_blk){
				goto out;	
			}
			else{
				(*conti) --;
			...
			random_value = get_random_int() % 100 ;

			if(degree > random_value){ //fragmentation case
				(*conti) = 0;
				if((*counter) >= 16 * spp->pgs_per_oneshotpg){
					r = vmalloc(sizeof(struct rewrite_pointer)); 
					r->ch = wpp->ch;
	...
	uint64_t i,j;
     for (i = 0; i < spp->nchs; i++) {
         for (j = 0; j < spp->luns_per_ch; j++) {
             uint64_t reset_idx = i * spp->luns_per_ch + j;
			// printk(KERN_INFO "ch: %d lun: %d pg: %d\n",i,j,conv_ftl->page_counter[reset_idx]);
             NVMEV_ASSERT(conv_ftl->page_counter[reset_idx] == spp->pgs_per_blk);
             conv_ftl->page_counter[reset_idx] = 0;
         }
     }
...
/*struct rewrite_pointer * phfrag_cleaner(struct nvmev_ns *ns, uint64_t start_lpn,uint64_t end_lpn,uint64_t degree, int idx, bool force, bool no_return,int * con, long num_list, int cur_chunk, int * conti){

	struct conv_ftl *conv_ftls = (struct conv_ftl *)ns->ftls;
	struct conv_ftl *conv_ftl = &conv_ftls[idx];
	...
	for (lpn = start_lpn; lpn <= end_lpn; lpn+=nr_parts) {
		...
		bool final=false;
		if(lpn+nr_parts > end_lpn)
			final=true;

		r = advance_write_pointer_cleanver(conv_ftl,degree,no_return,con,conti,final);
...
struct rewrite_pointer * phfrag_cleaner(struct nvmev_ns *ns, uint64_t start_lpn,uint64_t end_lpn,uint64_t degree,struct list_head * preempt_list, int idx, bool force, bool no_return,int * con, long num_list, int cur_chunk, int * conti){
	...
	if(!force && list_empty(&preempt_list[idx * num_list + cur_chunk])){
		force=true;
	}
	for (lpn = start_lpn; lpn <= end_lpn; lpn+=nr_parts) {
		if(force == false && no_return == false){
			struct write_pointer *wpp = __get_wp(conv_ftl, USER_IO);
			int premmpt_counter = 0;
			lba_chunk_t * prem,*tmp;

    		list_for_each_entry_safe(prem, tmp, &preempt_list[idx * num_list + cur_chunk], list) {
				if(wpp->ch == prem->ch && wpp->lun == prem->lun){
					r = vmalloc(sizeof(struct rewrite_pointer));
					r->ch = -1;
					r->lun = -1;
					r->startpoint = lpn;
					r->preempt = true;
					r->preempt_counter = premmpt_counter;
					return r;
				}
				premmpt_counter++;
			}
		...
		if(r != NULL){
			r->startpoint = lpn + nr_parts;
			if(r->startpoint > end_lpn){
				vfree(r);
				return NULL;
			}
			r->preempt = false;
			return r;
		}else if(*con >= 16 * spp->pgs_per_oneshotpg){
			if((lpn+nr_parts) > end_lpn){
				return NULL;
			}
			r = vmalloc(sizeof(struct rewrite_pointer));
...
void remove_ftl_shortcut(struct conv_ftl *conv_ftls, uint64_t nr_parts){
	uint32_t i;
	for(i=0; i < nr_parts;i++){
		remove_lines(&conv_ftls[i]);
		remove_rmap(&conv_ftls[i]);
		remove_maptbl(&conv_ftls[i]);
	}
}
void init_ftl_shortcut(struct conv_ftl *conv_ftls, uint64_t nr_parts){
	uint32_t i;
	for(i=0; i < nr_parts;i++){
		init_rmap(&conv_ftls[i]);
		init_maptbl(&conv_ftls[i]);
		init_lines(&conv_ftls[i]);
		prepare_write_pointer(&conv_ftls[i], USER_IO);
		prepare_write_pointer(&conv_ftls[i], GC_IO);
		init_write_flow_control(&conv_ftls[i]);
	}
}
