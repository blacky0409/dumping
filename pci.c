...
static void __process_msi_irq(struct nvmev_dev *nvmev_vdev, int msi_index)
{
	...
	struct nvmev_dev BUG_ON(virq == 0);
...
static void __init_nvme_ctrl_regs(struct nvmev_dev *ndev, struct pci_dev *dev)
{
...
static struct pci_bus *__create_pci_bus(struct nvmev_dev *nvmev_vdev)
{
	...
	bus = pci_scan_bus(NVMEV_PCI_BUS_NUM + pci_id_make, &nvmev_pci_ops,
			   &nvmev_vdev->pci_sysdata);
	pci_id_make++;
	if (!bus) {
	...
	/* Only support a singe NVMeVirt instance on a bus */
